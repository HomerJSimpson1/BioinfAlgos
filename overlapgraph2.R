...
printgraph <- function(graphdf) {
  ...
  for (i in 1:nrow(graphdf)) {
    #if (graphdf$patterns[i] != NULL)
    #if (graphdf$patterns[i] != "")
    #if (length(graphdf$adjacencylist[i]) > 0)
    #if (unlist(graphdf$adjacencylist[i]) != "")
    #if (length(unlist(graphdf$adjacencylist[i])) > 0)
    if (length(graphdf$adjacencylist[[i]]) > 0) {
      #output <- paste(output, graphdf$patterns[i], " -> ", unlist(graphdf$adjacencylist[i]), "\n", sep="")
      output <- paste(output, graphdf$patterns[i], " -> ", graphdf$adjacencylist[[i]], "\n", sep="")
    }
    #if (length(graphdf$adjacencylist[[i]]) == 0) print(i)

  }
  
  #print(graphdf$adjacencylist[[7847]])
