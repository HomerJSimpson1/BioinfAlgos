...
##   }
  ...
  ## writestringtofile("debruijnout.txt", adjlist)
  ## return(adjlist)
  
  #graph <- as.data.frame(cbind(prefixes, suffixes, adjlist), stringsAsFactors=FALSE)
...
printdebgraph <- function(graphdf) {
  ...
  for (i in 1:nrow(graphdf)) {
    ...
    if (length(graphdf$adjlist[[i]]) > 0) {      
      ...
      #output <- paste(output, graphdf$prefixes[i], " -> ", paste(graphdf$adjacencylist[[i]], "",sep="",collapse=","), "\n", sep="")
      #output <- paste(output, graphdf$prefixes[i], " -> ", paste(graphdf$adjlist[[i]], "",sep="",collapse=","), "\n", sep="")
